...
| AC17 | pass | three real conflicts produced six replacement pairs and twelve active rows; five idle originals remained SHA-identical and the active repair task retained its complete pre-migration byte prefix while Codex appended normally |
| AC18 | pass | the isolated single-conflict fixture covers every persisted phase, prepared-prefix recovery after source growth, target/catalog collisions, marker-correct orphan data, pre/post-snapshot rename refusal, semantic/legacy-state refusal, missing counterpart recovery, branch-local archive, and both SQLite integrity checks; the real `all` run completed three migrations |
| AC19 | pass | all three real migrations are `verified`; three old raw edges are suppressed; content conflicts remaining 0; twelve catalog/state titles match; both SQLite checks `ok`; one 29 MiB automatic snapshot and zero temporary backup files remain; isolated no-op coverage retains the same snapshot, while the second real run legitimately replaced it after syncing two unrelated active tasks |
